# Hi, I'm Emir Cantez 👋

Computer Science student at **Eötvös Loránd University (ELTE), Budapest**.  
I build practical web projects and improve my software engineering skills every week.

- 🌐 Focus: **JavaScript, React, HTML/CSS, C# basics**
- 🧠 Currently improving: **algorithms, clean code, problem solving**
- 🎯 Goal: **Internship / Junior Web Developer**
- 🤝 Open to: **Erasmus collaboration and project teamwork**

## Featured Work
- **Portfolio** – personal website and project showcase  
- **Exchange** – JavaScript-based exchange logic practice  
- **Calc** – calculator practice project  
- **E-bike** – responsive frontend practice

> I will keep adding structured school tasks and project-based work here.

## Learning Progress
I document selected university programming tasks in a clean, reproducible format:
- problem statement
- solution approach
- final code
- what I learned

## Contact
- Portfolio: https://cntzemirr.netlify.app/
- LinkedIn: https://www.linkedin.com/in/emircntz
